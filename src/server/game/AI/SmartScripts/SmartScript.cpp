...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_FLEE_FOR_ASSIST:
        {
            ...
            {
				Tournament::BroadcastTextBuilder builder(me, CHAT_MSG_MONSTER_EMOTE, BROADCAST_TEXT_FLEE_FOR_ASSIST);
        ...
        case SMART_ACTION_CALL_FOR_HELP:
        {
            if (me)
            {
                ...
                {
					Tournament::BroadcastTextBuilder builder(me, CHAT_MSG_MONSTER_EMOTE, BROADCAST_TEXT_CALL_FOR_HELP);
                    sCreatureTextMgr->SendChatPacket(me, builder, CHAT_MSG_MONSTER_EMOTE);
                }
...
ObjectList* SmartScript::GetWorldObjectsInDist(float dist)
{
    ...
    if (obj)
    {
		Tournament::AllWorldObjectsInRange u_check(obj, dist);
		Tournament::WorldObjectListSearcher<Tournament::AllWorldObjectsInRange> searcher(obj, *targets, u_check);
...
Unit* SmartScript::DoSelectLowestHpFriendly(float range, uint32 MinHPDiff)
{
	...
	CellCoord p(Tournament::ComputeCellCoord(me->GetPositionX(), me->GetPositionY()));
    Cell cell(p);
    cell.SetNoCreate();

    Unit* unit = NULL;

	Tournament::MostHPMissingInRange u_check(me, range, MinHPDiff);
	Tournament::UnitLastSearcher<Tournament::MostHPMissingInRange> searcher(me, unit, u_check);

	TypeContainerVisitor<Tournament::UnitLastSearcher<Tournament::MostHPMissingInRange>, GridTypeMapContainer >  grid_unit_searcher(searcher);
...
void SmartScript::DoFindFriendlyCC(std::list<Creature*>& _list, float range)
{
	...
	Tournament::FriendlyCCedInRange u_check(me, range);
	Tournament::CreatureListSearcher<Tournament::FriendlyCCedInRange> searcher(me, _list, u_check);

	TypeContainerVisitor<Tournament::CreatureListSearcher<Tournament::FriendlyCCedInRange>, GridTypeMapContainer >  grid_creature_searcher(searcher);
...
void SmartScript::DoFindFriendlyMissingBuff(std::list<Creature*>& list, float range, uint32 spellid)
{
	...
	Tournament::FriendlyMissingBuffInRange u_check(me, range, spellid);
	Tournament::CreatureListSearcher<Tournament::FriendlyMissingBuffInRange> searcher(me, list, u_check);

	TypeContainerVisitor<Tournament::CreatureListSearcher<Tournament::FriendlyMissingBuffInRange>, GridTypeMapContainer >  grid_creature_searcher(searcher);
...
Unit* SmartScript::DoFindClosestFriendlyInRange(float range, bool playerOnly)
{
    ...
    Unit* unit = NULL;
	Tournament::AnyFriendlyUnitInObjectRangeCheck u_check(me, me, range, playerOnly);
	Tournament::UnitLastSearcher<Tournament::AnyFriendlyUnitInObjectRangeCheck> searcher(me, unit, u_check);
