...
bool BfCapturePoint::Update(uint32 diff)
{
    ...
    if (GameObject* capturePoint = m_Bf->GetGameObject(m_capturePointGUID))
    {
		...
		Tournament::AnyPlayerInObjectRangeCheck checker(capturePoint, radius);
		Tournament::PlayerListSearcher<Tournament::AnyPlayerInObjectRangeCheck> searcher(capturePoint, players, checker);
