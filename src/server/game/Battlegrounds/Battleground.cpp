...
namespace Tournament
{
    class BattlegroundChatBuilder
    {
        public:
            ...
            {
				char const* text = sObjectMgr->GetTournamentString(_textId, loc_idx);
                if (_args)
                {
    ...
    class Battleground2ChatBuilder
    {
        public:
            ...
            {
				char const* text = sObjectMgr->GetTournamentString(_textId, loc_idx);
				char const* arg1str = _arg1 ? sObjectMgr->GetTournamentString(_arg1, loc_idx) : "";
				char const* arg2str = _arg2 ? sObjectMgr->GetTournamentString(_arg2, loc_idx) : "";
...
uint32 Battleground::GetBonusHonorFromKill(uint32 kills) const
{
	...
	return Tournament::Honor::hk_honor_at_level(maxLevel, float(kills));
}
...
void Battleground::SendMessageToAll(uint32 entry, ChatMsg type, Player const* source)
{
	...
	Tournament::BattlegroundChatBuilder bg_builder(type, entry, source);
	Tournament::LocalizedPacketDo<Tournament::BattlegroundChatBuilder> bg_do(bg_builder);
...
void Battleground::PSendMessageToAll(uint32 entry, ChatMsg type, Player const* source, ...)
{
	...
	Tournament::BattlegroundChatBuilder bg_builder(type, entry, source, &ap);
	Tournament::LocalizedPacketDo<Tournament::BattlegroundChatBuilder> bg_do(bg_builder);
    BroadcastWorker(bg_do);
...
void Battleground::SendWarningToAll(uint32 entry, ...)
{
    ...
    for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
        if (Player* player = _GetPlayer(itr, "SendWarningToAll"))
        {
            ...
            {
				char const* format = sObjectMgr->GetTournamentString(entry, player->GetSession()->GetSessionDbLocaleIndex());
...
{
	Tournament::Battleground2ChatBuilder bg_builder(type, entry, source, arg1, arg2);
	Tournament::LocalizedPacketDo<Tournament::Battleground2ChatBuilder> bg_do(bg_builder);
    BroadcastWorker(bg_do);
}
