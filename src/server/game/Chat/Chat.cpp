...
char const* ChatHandler::GetTournamentString(uint32 entry) const
{
	return m_session->GetTournamentString(entry);
}
...
{
	SendSysMessage(GetTournamentString(entry));
}
...
GameObject* ChatHandler::GetNearbyGameObject()
{
    ...
    GameObject* obj = NULL;
	Tournament::NearestGameObjectCheck check(*pl);
	Tournament::GameObjectLastSearcher<Tournament::NearestGameObjectCheck> searcher(pl, obj, check);
...
GameObject* ChatHandler::GetObjectGlobalyWithGuidOrNearWithDbGuid(uint32 lowguid, uint32 entry)
{
    ...
    if (!obj && sObjectMgr->GetGOData(lowguid))                   // guid is DB guid of object
    {
		...
		CellCoord p(Tournament::ComputeCellCoord(pl->GetPositionX(), pl->GetPositionY()));
        Cell cell(p);

		Tournament::GameObjectWithDbGUIDCheck go_check(*pl, lowguid);
		Tournament::GameObjectSearcher<Tournament::GameObjectWithDbGUIDCheck> checker(pl, obj, go_check);

		TypeContainerVisitor<Tournament::GameObjectSearcher<Tournament::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > object_checker(checker);
...
char const* CliHandler::GetTournamentString(uint32 entry) const
{
	return sObjectMgr->GetTournamentStringForDBCLocale(entry);
}
...
{
	return GetTournamentString(LANG_CONSOLE_COMMAND);
}
