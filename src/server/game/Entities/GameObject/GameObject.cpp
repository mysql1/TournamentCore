            ...
            if (isSpawned())
            {
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    if (Unit* owner = GetOwner())
                    {
						...
						Tournament::AnyUnfriendlyNoTotemUnitInObjectRangeCheck checker(this, owner, radius);
						Tournament::UnitSearcher<Tournament::AnyUnfriendlyNoTotemUnitInObjectRangeCheck> searcher(this, target, checker);
                    ...
                    else
                    {
                        ...
                        Player* player = NULL;
						Tournament::AnyPlayerInObjectRangeCheck checker(this, radius);
						Tournament::PlayerSearcher<Tournament::AnyPlayerInObjectRangeCheck> searcher(this, player, checker);
                        VisitNearbyWorldObject(radius, searcher);
                        target = player;
                    }
...
void GameObject::TriggeringLinkedGameObject(uint32 trapEntry, Unit* target)
{
    ...
    GameObject* trapGO = NULL;
    {
		...
		Tournament::NearestGameObjectEntryInObjectRangeCheck go_check(*target, trapEntry, range);
		Tournament::GameObjectLastSearcher<Tournament::NearestGameObjectEntryInObjectRangeCheck> checker(this, trapGO, go_check);

		TypeContainerVisitor<Tournament::GameObjectLastSearcher<Tournament::NearestGameObjectEntryInObjectRangeCheck>, GridTypeMapContainer > object_checker(checker);
...
GameObject* GameObject::LookupFishingHoleAround(float range)
{
	...
	CellCoord p(Tournament::ComputeCellCoord(GetPositionX(), GetPositionY()));
    Cell cell(p);
	Tournament::NearestGameObjectFishingHole u_check(*this, range);
	Tournament::GameObjectSearcher<Tournament::NearestGameObjectFishingHole> checker(this, ok, u_check);

	TypeContainerVisitor<Tournament::GameObjectSearcher<Tournament::NearestGameObjectFishingHole>, GridTypeMapContainer > grid_object_checker(checker);
            ...
            if (GetUniqueUseCount() == info->summoningRitual.reqParticipants)
            {
                ...
                if (info->summoningRitual.casterTargetSpell && info->summoningRitual.casterTargetSpell != 1) // No idea why this field is a bool in some cases
                    for (uint32 i = 0; i < info->summoningRitual.casterTargetSpellTargets; i++)
                        // m_unique_users can contain only player GUIDs
						if (Player* target = ObjectAccessor::GetPlayer(*this, Tournament::Containers::SelectRandomContainerElement(m_unique_users)))
