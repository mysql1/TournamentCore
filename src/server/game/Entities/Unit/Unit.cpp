...
void Unit::CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32* absorb, uint32* resist, SpellInfo const* spellInfo /*= NULL*/)
{
	...
	vSchoolAbsorbCopy.sort(Tournament::AbsorbAuraOrderPred());
...
Unit* Unit::SelectNearbyTarget(Unit* exclude, float dist) const
{
	...
	Tournament::AnyUnfriendlyUnitInObjectRangeCheck u_check(this, this, dist);
	Tournament::UnitListSearcher<Tournament::AnyUnfriendlyUnitInObjectRangeCheck> searcher(this, targets, u_check);
    ...
    // select random
	return Tournament::Containers::SelectRandomContainerElement(targets);
}
...
void Unit::UpdateObjectVisibility(bool forced)
{
    ...
    else
    {
		...
		Tournament::AIRelocationNotifier notifier(*this);
...
bool Unit::UpdatePosition(float x, float y, float z, float orientation, bool teleport)
{
	...
	if (!Tournament::IsValidMapCoord(x, y, z, orientation))
    {
...
{
	Tournament::CustomChatTextBuilder builder(this, msgType, text, language, target);
	Tournament::LocalizedPacketDo<Tournament::CustomChatTextBuilder> localizer(builder);
	Tournament::PlayerDistWorker<Tournament::LocalizedPacketDo<Tournament::CustomChatTextBuilder> > worker(this, textRange, localizer);
...
void Unit::Talk(uint32 textId, ChatMsg msgType, float textRange, WorldObject const* target)
{
	...
	Tournament::BroadcastTextBuilder builder(this, msgType, textId, target);
	Tournament::LocalizedPacketDo<Tournament::BroadcastTextBuilder> localizer(builder);
	Tournament::PlayerDistWorker<Tournament::LocalizedPacketDo<Tournament::BroadcastTextBuilder> > worker(this, textRange, localizer);
    VisitNearbyWorldObject(textRange, worker);
}
