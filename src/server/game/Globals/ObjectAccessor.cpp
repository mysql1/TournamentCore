...
template<class T> T* ObjectAccessor::GetObjectInWorld(uint32 mapid, float x, float y, ObjectGuid guid, T* /*fake*/)
{
	...
	CellCoord q = Tournament::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
    if (!q.IsCoordValid())
    {
...
void ObjectAccessor::RemoveCorpse(Corpse* corpse)
{
    ...
    // Critical section
    {
		...
		CellCoord cellCoord = Tournament::ComputeCellCoord(corpse->GetPositionX(), corpse->GetPositionY());
