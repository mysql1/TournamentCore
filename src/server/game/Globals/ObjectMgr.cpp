...
void ObjectMgr::AddCreatureToGrid(uint32 guid, CreatureData const* data)
{
    ...
    for (uint8 i = 0; mask != 0; i++, mask >>= 1)
    {
        if (mask & 1)
        {
			CellCoord cellCoord = Tournament::ComputeCellCoord(data->posX, data->posY);
    ...
    do
    {
        ...
        switch (tmp.command)
        {
            ...
            case SCRIPT_COMMAND_TELEPORT_TO:
            {
				...
				if (!Tournament::IsValidMapCoord(tmp.TeleportTo.DestX, tmp.TeleportTo.DestY, tmp.TeleportTo.DestZ, tmp.TeleportTo.Orientation))
                {
            ...
            {
				if (!Tournament::IsValidMapCoord(tmp.TempSummonCreature.PosX, tmp.TempSummonCreature.PosY, tmp.TempSummonCreature.PosZ, tmp.TempSummonCreature.Orientation))
                {
...
void ObjectMgr::LoadPointsOfInterest()
{
    ...
    do
    {
		...
		if (!Tournament::IsValidMapCoord(POI.x, POI.y))
        {
...
bool ObjectMgr::LoadTournamentStrings()
{
    uint32 oldMSTime = getMSTime();

	_TournamentStringStore.clear(); // for reload case

    QueryResult result = WorldDatabase.Query("SELECT entry, content_default, content_loc1, content_loc2, content_loc3, content_loc4, content_loc5, content_loc6, content_loc7, content_loc8 FROM tournament_string");
    if (!result)
    {
        TC_LOG_ERROR("server.loading", ">> Loaded 0 tournament strings. DB table `tournament_string` is empty.");
        return false;
    }
    ...
    do
    {
		...
		TournamentString& data = _TournamentStringStore[entry];
	...
	TC_LOG_INFO("server.loading", ">> Loaded " SZFMTD " Tournament strings in %u ms", _TournamentStringStore.size(), GetMSTimeDiffToNow(oldMSTime));
    return true;
}

char const* ObjectMgr::GetTournamentString(uint32 entry, LocaleConstant locale) const
{
	if (TournamentString const* ts = GetTournamentString(entry))
    {
    ...
    TC_LOG_ERROR("sql.sql", "Tournament string entry %u not found in DB.", entry);
    return "<error>";
}
...
{
	if (entry >= TOURNAMENT_TRAINER_START_REF)
        return;
