...
// Tournament Trainer Reference start range
#define TOURNAMENT_TRAINER_START_REF      200000

struct TournamentString
{
...
typedef std::unordered_map<uint32, TournamentString> TournamentStringContainer;
...
class ObjectMgr
{
    ...
    public:
        ...
        */
		...
		TournamentString const* GetTournamentString(uint32 entry) const
        {
			TournamentStringContainer::const_iterator itr = _TournamentStringStore.find(entry);
			if (itr == _TournamentStringStore.end())
                return nullptr;
            return &itr->second;
        }
		char const* GetTournamentString(uint32 entry, LocaleConstant locale) const;
		char const* GetTournamentStringForDBCLocale(uint32 entry) const { return GetTournamentString(entry, DBCLocaleIndex); }
    ...
    private:
		...
		TournamentStringContainer _TournamentStringStore;
