...
#ifndef TOURNAMENT_CELLIMPL_H
#define TOURNAMENT_CELLIMPL_H
...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
		CellCoord center = Tournament::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

	CellCoord centerX = Tournament::ComputeCellCoord(x - radius, y - radius).normalize();
	CellCoord centerY = Tournament::ComputeCellCoord(x + radius, y + radius).normalize();
