...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recvData)
{
    ...
    else
    {
        ...
        if (!sender->CanSpeak())
        {
			...
			SendNotification(GetTournamentString(LANG_WAIT_BEFORE_SPEAKING), timeStr.c_str());
    ...
    {
		SendNotification(GetTournamentString(LANG_GM_SILENCE), sender->GetName().c_str());
        recvData.rfinish();
        return;
    }
    ...
    switch (type)
    {
        ...
        case CHAT_MSG_YELL:
        {
            ...
            {
				SendNotification(GetTournamentString(LANG_SAY_REQ), sWorld->getIntConfig(CONFIG_CHAT_SAY_LEVEL_REQ));
                return;
            }
        ...
        case CHAT_MSG_WHISPER:
        {
            ...
            {
				SendNotification(GetTournamentString(LANG_WHISPER_REQ), sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ));
                return;
            }
            ...
            {
				SendNotification(GetTournamentString(LANG_GM_SILENCE), GetPlayer()->GetName().c_str());
                return;
            }
        ...
        case CHAT_MSG_CHANNEL:
        {
            if (!HasPermission(rbac::RBAC_PERM_SKIP_CHECK_CHAT_CHANNEL_REQ))
            {
                ...
                {
					SendNotification(GetTournamentString(LANG_CHANNEL_REQ), sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_LEVEL_REQ));
                    return;
                }
            }
        ...
        case CHAT_MSG_AFK:
        {
            if (!sender->IsInCombat())
            {
                ...
                {
					sender->autoReplyMsg = msg.empty() ? GetTournamentString(LANG_PLAYER_AFK_DEFAULT) : msg;
        ...
        case CHAT_MSG_DND:
        {
            ...
            {
				sender->autoReplyMsg = msg.empty() ? GetTournamentString(LANG_PLAYER_DND_DEFAULT) : msg;
...
void WorldSession::HandleTextEmoteOpcode(WorldPacket& recvData)
{
	...
	CellCoord p = Tournament::ComputeCellCoord(GetPlayer()->GetPositionX(), GetPlayer()->GetPositionY());

    Cell cell(p);
    cell.SetNoCreate();

	Tournament::EmoteChatBuilder emote_builder(*GetPlayer(), text_emote, emoteNum, unit);
	Tournament::LocalizedPacketDo<Tournament::EmoteChatBuilder > emote_do(emote_builder);
	Tournament::PlayerDistWorker<Tournament::LocalizedPacketDo<Tournament::EmoteChatBuilder > > emote_worker(GetPlayer(), sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE), emote_do);
	TypeContainerVisitor<Tournament::PlayerDistWorker<Tournament::LocalizedPacketDo<Tournament::EmoteChatBuilder> >, WorldTypeMapContainer> message(emote_worker);
