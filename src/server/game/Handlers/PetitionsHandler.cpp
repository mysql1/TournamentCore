...
void WorldSession::HandlePetitionQueryOpcode(WorldPacket& recvData)
{
    ...
    uint32 guildguid;
    ObjectGuid petitionguid;
    recvData >> guildguid;                                 // in Tournament always same as GUID_LOPART(petitionguid)
    recvData >> petitionguid;                              // petition guid
...
void WorldSession::SendPetitionQueryOpcode(ObjectGuid petitionguid)
{
    ...
    data << uint32(petitionguid.GetCounter());              // guild/team guid (in Tournament always same as GUID_LOPART(petition guid)
