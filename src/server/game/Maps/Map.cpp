...
void Map::SwitchGridContainers(Creature* obj, bool on)
{
	...
	CellCoord p = Tournament::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
    if (!p.IsCoordValid())
    {
...
{
	CellCoord cellCoord = Tournament::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
...
bool Map::AddToMap(T* obj)
{
	...
	CellCoord cellCoord = Tournament::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
...
void Map::VisitNearbyCellsOf(WorldObject* obj, TypeContainerVisitor<Tournament::ObjectUpdater, GridTypeMapContainer> &gridVisitor, TypeContainerVisitor<Tournament::ObjectUpdater, WorldTypeMapContainer> &worldVisitor)
{
...
void Map::Update(const uint32 t_diff)
{
	...
	Tournament::ObjectUpdater updater(t_diff);
    // for creature
	TypeContainerVisitor<Tournament::ObjectUpdater, GridTypeMapContainer  > grid_object_update(updater);
    // for pets
	TypeContainerVisitor<Tournament::ObjectUpdater, WorldTypeMapContainer > world_object_update(updater);
    ...
    for (GridRefManager<NGridType>::iterator i = GridRefManager<NGridType>::begin(); i != GridRefManager<NGridType>::end(); ++i)
    {
        ...
        for (uint32 x = cell_min.x_coord; x < cell_max.x_coord; ++x)
        {
            for (uint32 y = cell_min.y_coord; y < cell_max.y_coord; ++y)
            {
				...
				Tournament::DelayedUnitRelocation cell_relocation(cell, pair, *this, MAX_VISIBILITY_DISTANCE);
				TypeContainerVisitor<Tournament::DelayedUnitRelocation, GridTypeMapContainer  > grid_object_relocation(cell_relocation);
				TypeContainerVisitor<Tournament::DelayedUnitRelocation, WorldTypeMapContainer > world_object_relocation(cell_relocation);
...
{
    cell.SetNoCreate();
	Tournament::VisibleChangesNotifier notifier(*obj);
	TypeContainerVisitor<Tournament::VisibleChangesNotifier, WorldTypeMapContainer > player_notifier(notifier);
...
{
	Tournament::VisibleNotifier notifier(*player);

    cell.SetNoCreate();
	TypeContainerVisitor<Tournament::VisibleNotifier, WorldTypeMapContainer > world_notifier(notifier);
	TypeContainerVisitor<Tournament::VisibleNotifier, GridTypeMapContainer  > grid_notifier(notifier);
...
bool Map::ActiveObjectsNearGrid(NGridType const& ngrid) const
{
    ...
    for (MapRefManager::const_iterator iter = m_mapRefManager.begin(); iter != m_mapRefManager.end(); ++iter)
    {
		...
		CellCoord p = Tournament::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
...
void Map::AddToActive(Creature* c)
{
    ...
    if (!c->IsPet() && c->GetDBTableGUIDLow())
    {
		...
		GridCoord p = Tournament::ComputeGridCoord(x, y);
        if (getNGrid(p.x_coord, p.y_coord))
            getNGrid(p.x_coord, p.y_coord)->incUnloadActiveLock();
        else
        {
			GridCoord p2 = Tournament::ComputeGridCoord(c->GetPositionX(), c->GetPositionY());
