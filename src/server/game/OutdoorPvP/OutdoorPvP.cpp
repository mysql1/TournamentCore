...
bool OPvPCapturePoint::Update(uint32 diff)
{
	...
	Tournament::AnyPlayerInObjectRangeCheck checker(m_capturePoint, radius);
	Tournament::PlayerListSearcher<Tournament::AnyPlayerInObjectRangeCheck> searcher(m_capturePoint, players, checker);
...
void OutdoorPvP::SendDefenseMessage(uint32 zoneId, uint32 id)
{
	...
	Tournament::LocalizedPacketDo<DefenseMessageBuilder> localizer(builder);
