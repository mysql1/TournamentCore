...
inline GameObject* Map::_FindGameObject(WorldObject* searchObject, uint32 guid) const
{
	...
	CellCoord p(Tournament::ComputeCellCoord(searchObject->GetPositionX(), searchObject->GetPositionY()));
    Cell cell(p);

	Tournament::GameObjectWithDbGUIDCheck goCheck(*searchObject, guid);
	Tournament::GameObjectSearcher<Tournament::GameObjectWithDbGUIDCheck> checker(searchObject, gameobject, goCheck);

	TypeContainerVisitor<Tournament::GameObjectSearcher<Tournament::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > objectChecker(checker);
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= sWorld->GetGameTime()))
    {
        ...
        switch (step.script->command)
        {
            ...
            case SCRIPT_COMMAND_CALLSCRIPT_TO_UNIT:
            {
                ...
                {
					CellCoord p(Tournament::ComputeCellCoord(wSource->GetPositionX(), wSource->GetPositionY()));
                    Cell cell(p);

					Tournament::CreatureWithDbGUIDCheck target_check(wSource, step.script->CallScript.CreatureEntry);
					Tournament::CreatureSearcher<Tournament::CreatureWithDbGUIDCheck> checker(wSource, cTarget, target_check);

					TypeContainerVisitor<Tournament::CreatureSearcher <Tournament::CreatureWithDbGUIDCheck>, GridTypeMapContainer > unit_checker(checker);
