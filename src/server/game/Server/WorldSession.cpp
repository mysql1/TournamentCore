...
{
	char const* format = GetTournamentString(string_id);
    if (format)
    {
        va_list ap;
        char szStr[1024];
...
char const* WorldSession::GetTournamentString(uint32 entry) const
{
	return sObjectMgr->GetTournamentString(entry, GetSessionDbLocaleIndex());
}
