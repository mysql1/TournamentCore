...
// such checks will speedup tournament change amount/send for client operations
...
void AuraEffect::HandleFeignDeath(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        */
        ...
        UnitList targets;
		Tournament::AnyUnfriendlyUnitInObjectRangeCheck u_check(target, target, target->GetMap()->GetVisibilityRange());
		Tournament::UnitListSearcher<Tournament::AnyUnfriendlyUnitInObjectRangeCheck> searcher(target, targets, u_check);
