...
void Spell::SelectImplicitConeTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    {
		Tournament::WorldObjectSpellConeTargetCheck check(coneAngle, radius, m_caster, m_spellInfo, selectionType, condList);
		Tournament::WorldObjectListSearcher<Tournament::WorldObjectSpellConeTargetCheck> searcher(m_caster, targets, check, containerTypeMask);
		SearchTargets<Tournament::WorldObjectListSearcher<Tournament::WorldObjectSpellConeTargetCheck> >(searcher, containerTypeMask, m_caster, m_caster, radius);
        ...
        if (!targets.empty())
        {
            ...
            if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            {
				...
				Tournament::Containers::RandomResizeList(targets, maxTargets);
            }
...
void Spell::SelectImplicitTrajTargets(SpellEffIndex effIndex)
{
    ...
    std::list<WorldObject*> targets;
	Tournament::WorldObjectSpellTrajTargetCheck check(dist2d, m_targets.GetSrcPos(), m_caster, m_spellInfo);
	Tournament::WorldObjectListSearcher<Tournament::WorldObjectSpellTrajTargetCheck> searcher(m_caster, targets, check, GRID_MAP_TYPE_MASK_ALL);
	SearchTargets<Tournament::WorldObjectListSearcher<Tournament::WorldObjectSpellTrajTargetCheck> >(searcher, GRID_MAP_TYPE_MASK_ALL, m_caster, m_targets.GetSrcPos(), dist2d);
    if (targets.empty())
        return;

	targets.sort(Tournament::ObjectDistanceOrderPred(m_caster));
...
void Spell::SearchTargets(SEARCHER& searcher, uint32 containerMask, Unit* referer, Position const* pos, float radius)
{
    ...
    if (searchInGrid || searchInWorld)
    {
		...
		CellCoord p(Tournament::ComputeCellCoord(x, y));
        Cell cell(p);
        cell.SetNoCreate();
...
WorldObject* Spell::SearchNearbyTarget(float range, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType, ConditionList* condList)
{
    ...
    if (!containerTypeMask)
        return NULL;
	Tournament::WorldObjectSpellNearbyTargetCheck check(range, m_caster, m_spellInfo, selectionType, condList);
	Tournament::WorldObjectLastSearcher<Tournament::WorldObjectSpellNearbyTargetCheck> searcher(m_caster, target, check, containerTypeMask);
	SearchTargets<Tournament::WorldObjectLastSearcher<Tournament::WorldObjectSpellNearbyTargetCheck> >(searcher, containerTypeMask, m_caster, m_caster, range);
    return target;
}
...
void Spell::SearchAreaTargets(std::list<WorldObject*>& targets, float range, Position const* position, Unit* referer, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType, ConditionList* condList)
{
    ...
    if (!containerTypeMask)
        return;
	Tournament::WorldObjectSpellAreaTargetCheck check(range, position, m_caster, referer, m_spellInfo, selectionType, condList);
	Tournament::WorldObjectListSearcher<Tournament::WorldObjectSpellAreaTargetCheck> searcher(m_caster, targets, check, containerTypeMask);
	SearchTargets<Tournament::WorldObjectListSearcher<Tournament::WorldObjectSpellAreaTargetCheck> >(searcher, containerTypeMask, m_caster, position, range);
}
...
GameObject* Spell::SearchSpellFocus()
{
	...
	Tournament::GameObjectFocusCheck check(m_caster, m_spellInfo->RequiresSpellFocus);
	Tournament::GameObjectSearcher<Tournament::GameObjectFocusCheck> searcher(m_caster, focus, check);
	SearchTargets<Tournament::GameObjectSearcher<Tournament::GameObjectFocusCheck> >(searcher, GRID_MAP_TYPE_MASK_GAMEOBJECT, m_caster, m_caster, m_caster->GetVisibilityRange());
    return focus;
}
