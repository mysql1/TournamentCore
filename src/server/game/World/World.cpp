...
void World::LoadConfigSettings(bool reload)
{
    ...
    SetMotd(sConfigMgr->GetStringDefault("Motd", "Welcome to a Tournament Core Server."));
...
void World::SetInitialWorldSettings()
{
    ...
    TC_LOG_INFO("server.loading", "Loading Tournament strings...");
	if (!sObjectMgr->LoadTournamentStrings())
...
namespace Tournament
{
    class WorldWorldTextBuilder
    {
        public:
            ...
            {
				char const* text = sObjectMgr->GetTournamentString(i_textId, loc_idx);
...
void World::SendWorldText(uint32 string_id, ...)
{
	...
	Tournament::WorldWorldTextBuilder wt_builder(string_id, &ap);
	Tournament::LocalizedPacketListDo<Tournament::WorldWorldTextBuilder> wt_do(wt_builder);
    for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
