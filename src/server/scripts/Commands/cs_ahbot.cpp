...
public:
    ...
    static bool HandleAHBotItemsAmountCommand(ChatHandler* handler, const char* args)
    {
        ...
        for (int i = 0; i < MAX_AUCTION_QUALITY; ++i)
			handler->PSendSysMessage(LANG_AHBOT_ITEMS_AMOUNT, handler->GetTournamentString(ahbotQualityIds[i]), sAuctionBotConfig->GetConfigItemQualityAmount(AuctionQuality(i)));
    ...
    static bool HandleAHBotItemsAmountQualityCommand(ChatHandler* handler, const char* args)
    {
		...
		handler->PSendSysMessage(LANG_AHBOT_ITEMS_AMOUNT, handler->GetTournamentString(ahbotQualityIds[Q]),
    ...
    static bool HandleAHBotStatusCommand(ChatHandler* handler, const char* args)
    {
		...
		handler->PSendSysMessage(fmtId, handler->GetTournamentString(LANG_AHBOT_STATUS_ITEM_COUNT),
        ...
        if (all)
        {
			handler->PSendSysMessage(fmtId, handler->GetTournamentString(LANG_AHBOT_STATUS_ITEM_RATIO),
            ...
            for (int i = 0; i < MAX_AUCTION_QUALITY; ++i)
				handler->PSendSysMessage(fmtId, handler->GetTournamentString(ahbotQualityIds[i]),
