...
public:
    ...
    static bool HandleDebugEnterVehicleCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
			...
			Tournament::AllCreaturesOfEntryInRange check(handler->GetSession()->GetPlayer(), entry, 20.0f);
			Tournament::CreatureSearcher<Tournament::AllCreaturesOfEntryInRange> searcher(handler->GetSession()->GetPlayer(), passenger, check);
