...
public:
    ...
    static bool HandleGPSCommand(ChatHandler* handler, char const* args)
    {
		...
		CellCoord cellCoord = Tournament::ComputeCellCoord(object->GetPositionX(), object->GetPositionY());
        Cell cell(cellCoord);
		...
		GridCoord gridCoord = Tournament::ComputeGridCoord(object->GetPositionX(), object->GetPositionY());
		...
		char const* unknown = handler->GetTournamentString(LANG_UNKNOWN);
    ...
    static bool HandleSummonCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
			...
			handler->PSendSysMessage(LANG_SUMMONING, nameLink.c_str(), handler->GetTournamentString(LANG_OFFLINE));
    ...
    static bool HandleCooldownCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            {
				handler->PSendSysMessage(LANG_UNKNOWN_SPELL, target == handler->GetSession()->GetPlayer() ? handler->GetTournamentString(LANG_YOU) : nameLink.c_str());
                handler->SetSentErrorMessage(true);
                return false;
            }

            target->RemoveSpellCooldown(spellIid, true);
			handler->PSendSysMessage(LANG_REMOVE_COOLDOWN, spellIid, target == handler->GetSession()->GetPlayer() ? handler->GetTournamentString(LANG_YOU) : nameLink.c_str());
        }
        return true;
    }
    ...
    static bool HandleKickPlayerCommand(ChatHandler* handler, char const* args)
    {
		...
		std::string kickReasonStr = handler->GetTournamentString(LANG_NO_REASON);
        if (*args != '\0')
        {
    ...
    static bool HandleNearGraveCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (graveyard)
        {
			...
			std::string team_name = handler->GetTournamentString(LANG_COMMAND_GRAVEYARD_NOTEAM);

            if (team == 0)
				team_name = handler->GetTournamentString(LANG_COMMAND_GRAVEYARD_ANY);
            else if (team == HORDE)
				team_name = handler->GetTournamentString(LANG_COMMAND_GRAVEYARD_HORDE);
            else if (team == ALLIANCE)
				team_name = handler->GetTournamentString(LANG_COMMAND_GRAVEYARD_ALLIANCE);
    ...
    static bool HandleAddItemCommand(ChatHandler* handler, char const* args)
    {
		...
		TC_LOG_DEBUG("misc", handler->GetTournamentString(LANG_ADDITEM), itemId, count);
    ...
    static bool HandleAddItemSetCommand(ChatHandler* handler, char const* args)
    {
		...
		TC_LOG_DEBUG("misc", handler->GetTournamentString(LANG_ADDITEMSET), itemSetId);
    ...
    **/
    ...
    static bool HandlePInfoCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string userName          = handler->GetTournamentString(LANG_ERROR);
        uint32 accId                  = 0;
        uint32 lowguid                = targetGuid.GetCounter();
        std::string eMail             = handler->GetTournamentString(LANG_ERROR);
        std::string regMail           = handler->GetTournamentString(LANG_ERROR);
        uint32 security               = 0;
        std::string lastIp            = handler->GetTournamentString(LANG_ERROR);
        uint8 locked                  = 0;
        std::string lastLogin         = handler->GetTournamentString(LANG_ERROR);
        uint32 failedLogins           = 0;
        uint32 latency                = 0;
        std::string OS                = handler->GetTournamentString(LANG_UNKNOWN);

        // Mute data print variables
        int64 muteTime                = -1;
        std::string muteReason        = handler->GetTournamentString(LANG_NO_REASON);
        std::string muteBy            = handler->GetTournamentString(LANG_UNKNOWN);

        // Ban data print variables
        int64 banTime                 = -1;
        std::string banType           = handler->GetTournamentString(LANG_UNKNOWN);
        std::string banReason         = handler->GetTournamentString(LANG_NO_REASON);
        std::string bannedBy          = handler->GetTournamentString(LANG_UNKNOWN);

        // Character data print variables
        uint8 raceid, classid           = 0; //RACE_NONE, CLASS_NONE
        std::string raceStr, classStr   = handler->GetTournamentString(LANG_UNKNOWN);
        uint8 gender                    = 0;
        int8 locale                     = handler->GetSessionDbcLocale();
        uint32 totalPlayerTime          = 0;
        uint8 level                     = 0;
        std::string alive               = handler->GetTournamentString(LANG_ERROR);
        uint32 money                    = 0;
        uint32 xp                       = 0;
        uint32 xptotal                  = 0;
        ...
        uint32 areaId;
        uint32 phase            = 0;
        std::string areaName    = handler->GetTournamentString(LANG_UNKNOWN);
        std::string zoneName    = handler->GetTournamentString(LANG_UNKNOWN);
        ...
        if (target)
        {
            ...
            alive             = target->IsAlive() ? handler->GetTournamentString(LANG_YES) : handler->GetTournamentString(LANG_NO);
        ...
        else
        {
            ...
            if (!health || playerFlags & PLAYER_FLAGS_GHOST)
				alive = handler->GetTournamentString(LANG_NO);
            else
				alive = handler->GetTournamentString(LANG_YES);
        }
        ...
        if (result)
        {
            ...
            {
                eMail     = handler->GetTournamentString(LANG_UNAUTHORIZED);
                regMail   = handler->GetTournamentString(LANG_UNAUTHORIZED);
                lastIp    = handler->GetTournamentString(LANG_UNAUTHORIZED);
                lastLogin = handler->GetTournamentString(LANG_UNAUTHORIZED);
            }
        ...
        if (!result2)
        {
			banType = handler->GetTournamentString(LANG_CHARACTER);
		...
		handler->PSendSysMessage(LANG_PINFO_PLAYER, target ? "" : handler->GetTournamentString(LANG_OFFLINE), nameLink.c_str(), lowguid);
        ...
        if (banTime >= 0)
			handler->PSendSysMessage(LANG_PINFO_BANNED, banType.c_str(), banReason.c_str(), banTime > 0 ? secsToTimeString(banTime - time(NULL), true).c_str() : handler->GetTournamentString(LANG_PERMANENTLY), bannedBy.c_str());
		...
		handler->PSendSysMessage(LANG_PINFO_ACC_IP, lastIp.c_str(), locked ? handler->GetTournamentString(LANG_YES) : handler->GetTournamentString(LANG_NO));
		...
		handler->PSendSysMessage(LANG_PINFO_CHR_RACE, (gender == 0 ? handler->GetTournamentString(LANG_CHARACTER_GENDER_MALE) : handler->GetTournamentString(LANG_CHARACTER_GENDER_FEMALE)), raceStr.c_str(), classStr.c_str());
        ...
        if (target)
            handler->PSendSysMessage(LANG_PINFO_CHR_MAP, map->name[locale],
                (!zoneName.empty() ? zoneName.c_str() : handler->GetTournamentString(LANG_UNKNOWN)),
                (!areaName.empty() ? areaName.c_str() : handler->GetTournamentString(LANG_UNKNOWN)));
    ...
    static bool HandleRespawnCommand(ChatHandler* handler, char const* /*args*/)
    {
		...
		Tournament::RespawnDo u_do;
		Tournament::WorldObjectWorker<Tournament::RespawnDo> worker(player, u_do);
    ...
    static bool HandleMuteCommand(ChatHandler* handler, char const* args)
    {
		...
		std::string muteReasonStr = handler->GetTournamentString(LANG_NO_REASON);
        if (muteReason != NULL)
        ...
        if (handler->GetSession())
			...
			muteBy = handler->GetTournamentString(LANG_CONSOLE);
