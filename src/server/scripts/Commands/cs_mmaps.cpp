...
public:
    ...
    static bool HandleMmapTestArea(ChatHandler* handler, char const* /*args*/)
    {
		...
		CellCoord pair(Tournament::ComputeCellCoord(object->GetPositionX(), object->GetPositionY()));
        Cell cell(pair);
        cell.SetNoCreate();

        std::list<Creature*> creatureList;

		Tournament::AnyUnitInObjectRangeCheck go_check(object, radius);
		Tournament::CreatureListSearcher<Tournament::AnyUnitInObjectRangeCheck> go_search(object, creatureList, go_check);
		TypeContainerVisitor<Tournament::CreatureListSearcher<Tournament::AnyUnitInObjectRangeCheck>, GridTypeMapContainer> go_visit(go_search);
