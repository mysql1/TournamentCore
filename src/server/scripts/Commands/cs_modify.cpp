...
public:
    ...
    static bool HandleModifyEnergyCommand(ChatHandler* handler, const char* args)
    {
		...
		TC_LOG_DEBUG("misc", handler->GetTournamentString(LANG_CURRENT_ENERGY), target->GetMaxPower(POWER_ENERGY));
    ...
    static bool HandleModifyMoneyCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (moneyToAdd < 0)
        {
			...
			TC_LOG_DEBUG("misc", handler->GetTournamentString(LANG_CURRENT_MONEY), targetMoney, moneyToAdd, newmoney);
            if (newmoney <= 0)
            {
		...
		TC_LOG_DEBUG("misc", handler->GetTournamentString(LANG_NEW_MONEY), targetMoney, moneyToAdd, target->GetMoney());
    ...
    static bool HandleModifyRepCommand(ChatHandler* handler, const char* args)
    {
        ...
        if ((amount == 0) && (rankTxt[0] != '-') && !isdigit(rankTxt[0]))
        {
            ...
            {
				std::string rank = handler->GetTournamentString(ReputationRankStrIndex[r]);
                if (rank.empty())
                    continue;
