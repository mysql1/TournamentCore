...
{
	CellCoord pair(Tournament::ComputeCellCoord(x, y));
    Cell cell(pair);
    cell.SetNoCreate();

    // First get all creatures.
    std::list<Creature*> creatures;
	Tournament::AllFriendlyCreaturesInGrid creature_check(me);
	Tournament::CreatureListSearcher<Tournament::AllFriendlyCreaturesInGrid> creature_searcher(me, creatures, creature_check);

	TypeContainerVisitor <Tournament::CreatureListSearcher<Tournament::AllFriendlyCreaturesInGrid>, GridTypeMapContainer> creature_visitor(creature_searcher);
...
void hyjalAI::RespawnNearPos(float x, float y)
{
	...
	Tournament::WorldObjectWorker<Tournament::RespawnDo> worker(me, u_do);
	TypeContainerVisitor<Tournament::WorldObjectWorker<Tournament::RespawnDo>, GridTypeMapContainer > obj_worker(worker);
...
void hyjalAI::WaypointReached(uint32 waypointId)
{
    if (waypointId == 1 || (waypointId == 0 && me->GetEntry() == THRALL))
    {
        ...
        TypeContainerVisitor
			<Tournament::CreatureListSearcher<Tournament::AllFriendlyCreaturesInGrid>,
