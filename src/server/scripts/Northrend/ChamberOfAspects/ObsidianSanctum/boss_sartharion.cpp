...
public:
    ...
    struct boss_sartharionAI : public BossAI
    {
        ...
        void CastLavaStrikeOnTarget(Unit* target)
        {
			...
			Tournament::AllCreaturesOfEntryInRange checker(me, NPC_FIRE_CYCLONE, 200.0f);
			Tournament::CreatureListSearcher<Tournament::AllCreaturesOfEntryInRange> searcher(me, fireCyclonesList, checker);
            me->VisitNearbyObject(200.0f, searcher);

            if (fireCyclonesList.empty())
                return;

			Tournament::Containers::SelectRandomContainerElement(fireCyclonesList)->CastSpell(target, SPELL_LAVA_STRIKE, true);
        }
