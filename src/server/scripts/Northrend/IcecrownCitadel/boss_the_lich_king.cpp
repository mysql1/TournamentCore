    ...
    public:
        ...
        struct boss_the_lich_kingAI : public BossAI
        {
            ...
            void JustReachedHome() override
            {
				...
				Tournament::GameObjectWorker<FrozenThroneResetWorker> worker(me, reset);
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_FROSTMOURNE_HEROIC:
                            if (TempSummon* terenas = me->GetMap()->SummonCreature(NPC_TERENAS_MENETHIL_FROSTMOURNE_H, TerenasSpawnHeroic, NULL, 50000))
                            {
                                ...
                                if (!triggers.empty())
                                {
									triggers.sort(Tournament::ObjectDistanceOrderPred(terenas, true));
                    ...
                    case POINT_CHARGE:
                        if (Player* target = ObjectAccessor::GetPlayer(*me, _grabbedPlayer))
                        {
                            ...
                            if (GameObject* platform = ObjectAccessor::GetGameObject(*me, _instance->GetGuidData(DATA_ARTHAS_PLATFORM)))
                            {
								...
								triggers.sort(Tournament::ObjectDistanceOrderPred(me));
...
class spell_the_lich_king_necrotic_plague_jump : public SpellScriptLoader
{
    public:
        ...
        private:
            ...
            {
				targets.sort(Tournament::ObjectDistanceOrderPred(GetCaster()));
                if (targets.size() < 2)
                    return;
    ...
    public:
        ...
        class spell_the_lich_king_defile_SpellScript : public SpellScript
        {
            ...
            void CorrectRange(std::list<WorldObject*>& targets)
            {
				...
				targets.remove_if(Tournament::UnitAuraCheck(true, SPELL_HARVEST_SOUL_VALKYR));
            }
    ...
    public:
        ...
        private:
            ...
            void SelectTarget(std::list<WorldObject*>& targets)
            {
				...
				targets.remove_if(Tournament::UnitAuraCheck(true, GetSpellInfo()->Id));
                if (targets.empty())
                    return;

				_target = Tournament::Containers::SelectRandomContainerElement(targets);
                targets.clear();
