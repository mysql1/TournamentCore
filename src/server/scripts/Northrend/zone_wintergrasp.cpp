        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            ...
            for (uint8 i = 0; i < graveyard.size(); i++)
                if (graveyard[i]->GetControlTeamId() == player->GetTeamId())
					player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetSession()->GetTournamentString(((BfGraveyardWG*)graveyard[i])->GetTextId()), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + i);
...
class npc_wg_queue : public CreatureScript
{
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            ...
            {
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, player->GetSession()->GetTournamentString(WG_NPCQUEUE_TEXTOPTION_JOIN), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
