...
#ifndef TOURNAMENTCORE_LOG_H
#define TOURNAMENTCORE_LOG_H
...
class Log
{
    ...
    public:
        ...
        {
			write(std::unique_ptr<LogMessage>(new LogMessage(level, filter, Tournament::StringFormat(fmt, args...))));
        }
        ...
        void outCommand(uint32 account, const char* fmt, Args const&... args)
        {
			...
			std::unique_ptr<LogMessage> msg(new LogMessage(LOG_LEVEL_INFO, "commands.gm", std::move(Tournament::StringFormat(fmt, args...))));
